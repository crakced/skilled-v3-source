...
public class BlockESP extends Module
{
    ...
    public void onRenderEvent(final RenderWorldLastEvent event) {
        ...
        for (final BlockESPBlock block : this.blockList) {
            ...
            RenderUtil.drawOutlinedBoundingBox(AxisAlignedBB.func_178781_a(x, y, z, x + 1.0, y + 1.0, z + 1.0));
        }
        GL11.glDisable(2848);
        GL11.glEnable(3553);
        GL11.glEnable(2929);
    ...
    private void search() {
        ...
        final WorldClient world = this.mc.field_71441_e;
        int y;
        for (int range = y = (int)this.range.getValue(); y >= -range; --y) {
            for (int x = range; x >= -range; --x) {
                for (int z = range; z >= -range; --z) {
                    final int posX = (int)(player.field_70165_t + x);
                    final int posY = (int)(player.field_70163_u + y);
                    final int posZ = (int)(player.field_70161_v + z);
                    final Block block = world.func_180495_p(new BlockPos(posX, posY, posZ)).func_177230_c();
                    final int id = Block.func_149682_b(block);
